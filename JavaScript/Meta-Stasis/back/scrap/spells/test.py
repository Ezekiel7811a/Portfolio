...
def getSpellInfo(href, element):
    ...
    if element == "special":
        if soup.find("div", {"class": "ak-level-selector-target ak-level-1 show"}):
            lvlSelector = soup.find("div", {"class": "ak-level-selector-target ak-level-1 show"})
        elif soup.find("div", {"class": "ak-level-selector-target ak-level-230 show"}):
            lvlSelector = soup.find("div", {"class": "ak-level-selector-target ak-level-230 show"})
        else:
            lvlSelector = soup.find("div", {"class": "ak-level-selector-target ak-level-1 hide"})
    else:
        lvlSelector = soup.find("div", {"class": "ak-level-selector-target ak-level-230 show"})
    # --------- CHOPPER L'IMAGE -----------------
    image = lvlSelector.find("img")["src"]
    id = image.split("/")[-1].split(".")[0].strip()
...
driver.get(url_dpt.format("2-osamodas"))
driver.implicitly_wait(2)
...
globalSorts.append({"2-osamodas".split("-")[1].strip(): sorts})

writing.writeData("spells.json", globalSorts, indent=3)